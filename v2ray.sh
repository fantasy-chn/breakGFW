...
function install_nginx(){
    ...
    tar xf nginx-1.18.0.tar.gz && rm nginx-1.18.0.tar.gz >/dev/null 2>&1
    cd nginx-1.18.0
...
server {
	...
	#替换网站内容
	    sub_filter 'www.baidu.com' '$your_domain';
    location /$newpath {
        proxy_redirect off;
...
function install_v2ray(){
    ...
    cd /etc/v2ray/
    rm -f config.json
    wget https://raw.githubusercontent.com/fantasy-chn/breakGFW/master/config.json >/dev/null 2>&1
    v2uuid=$(cat /proc/sys/kernel/random/uuid)
    sed -i "s/placeholder/$v2uuid/;" config.json
    sed -i "s/random/$newpath/;" config.json
